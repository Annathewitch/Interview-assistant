      ...
      <div style={contentStyle}>
        ...
        {bottomTab === "首页" && (
          <>
            ...
            {topTab === "面试" && <div style={emptyStyle}>日历组件加载中...</div>}
          </>
        )}
                ...
                {codingDemo ? (
                   <div style={editorOverlay}>
                    <div style={problemFloatingCard}>
                      <b>题目：两数之和</b>
                      <p style={{fontSize:'12px', marginTop:'5px'}}>给定 nums = [2, 7, 11, 15], target = 9。</p>
                    </div>
                    <textarea style={editorArea} defaultValue={`function twoSum(nums, target) {\n  // 编程 Demo 区域\n}`} />
                    <button onClick={() => setCodingDemo(false)} style={modalBtnStyle}>返回</button>
                   </div>
                ) : (
                  <div style={listStyle}>
                    <div style={cardStyle} onClick={() => setCodingDemo(true)}>
                       <div style={titleStyle}>LeetCode 1. 两数之和</div>
                       <div style={{fontSize:'12px', color:'#3B82F6', marginTop:'5px'}}>点击进入模拟编程</div>
                    </div>
                  </div>
                )}
              </div>
      ...
      {showGuide && (
        <div style={guideOverlay}>
          <div style={guideBubble}>
            <h3 style={{margin:'0 0 10px'}}>新手引导</h3>
            <p style={{fontSize:'14px', color:'#666'}}>欢迎使用！你目前还没有投递岗位，请点击下方的 <b style={{color:'#3B82F6'}}>“+”</b> 号创建第一个岗位记录吧。</p>
      ...
      {showJobForm && (
        <div style={modalStyle}>
          <div style={modalContentStyle}>
            ...
            <input style={inputStyle} placeholder="工作城市" value={location} onChange={e=>setLocation(e.target.value)} />
            <select style={inputStyle} value={status} onChange={e=>setStatus(e.target.value)}>
              {STATUS.map(s=><option key={s}>{s}</option>)}
            </select>
            <button onClick={addJob} style={modalBtnStyle}>保存并开启求职之旅</button>
          </div>
        </div>
      )}
