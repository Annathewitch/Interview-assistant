...
export default function Page() {
      ...
      if (currentInterview) {
        const updated = interviews.map(it =>
          it.id === currentInterview.id ? {
            ...it,
            transcript: realTimeTranscript || "等待转录...",
  ...
  const startRecording = async () => {
    ...
    (window as any).recordingTimer = timerRef;
  ...
  const stopRecording = () => {
    ...
    // 清理计时器
    if ((window as any).recordingTimer?.current) {
      clearInterval((window as any).recordingTimer.current);
  ...
  const stopSpeechRecognition = () => {
    if ((window as any).currentRecognition) {
      try {
        ((window as any).currentRecognition as SpeechRecognition).stop();
      } catch (e) {
        // 忽略错误
      }
              ...
              <div style={interviewPageStyle}>
                ...
                {selectedDay && currentInterview && (
                  <div style={interviewDetailCardStyle}>
                    ...
                    {!recording ? (
                      <button onClick={startRecording} style={recordBtnStyle}>
                        🎙️ 开始录音
                      </button>
                    ) : (
                      <div style={recordingControlsStyle}>
                        <button onClick={stopRecording} style={stopRecordBtnStyle}>
                          ⏹️ 停止录音
                        </button>
                        <div style={recordingTimerStyle}>
                    ...
                    <div style={transcriptCardStyle}>
                      ...
                      <div style={transcriptTextStyle}>
                        {realTimeTranscript || currentInterview.transcript || "暂无录音"}
                      </div>
                      {currentInterview.audioBlobUrl && !recording && (
                        <div style={audioPlayerStyle}>
                          ...
                          {currentInterview.recordingDuration && (
                            <div style={audioDurationStyle}>
                              时长: {formatTime(currentInterview.recordingDuration)}
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    {!recording && currentInterview && (currentInterview.transcript || currentInterview.audioData) && (
                      <button onClick={goToReport} style={reportBtnStyle}>🤖 AI复盘</button>
                    )}
                  </div>
                )}
              </div>
